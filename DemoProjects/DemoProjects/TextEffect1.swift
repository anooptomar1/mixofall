...
@IBDesignable public class TextEffect1: TextFieldEffects {
    
    private let foregroundView = UIView()
    private let foregroundLayer = CALayer()
    private let borderThickness: CGFloat = 0
    private let placeholderInsets = CGPointMake(6, 6)
    private let textFieldInsets = CGPointMake(6, 6)
    ...
    override public func animateViewsForTextEntry() {
        UIView.animateWithDuration(0.4, delay: 0, usingSpringWithDamping: 1.0, initialSpringVelocity: 0.6, options: UIViewAnimationOptions.BeginFromCurrentState, animations: { [weak self]() -> Void in
    ...
    override public func animateViewsForTextDisplay() {
        if text!.isEmpty{
            UIView.animateWithDuration(0.4, delay: 0, usingSpringWithDamping: 1.0, initialSpringVelocity: 0.6, options: UIViewAnimationOptions.BeginFromCurrentState, animations: { [weak self]() -> Void in
                ...
                self!.foregroundView.layer.transform = self!.rotateAndPerspectiveTransformForView(self!.foregroundView)
                }, completion: nil)
        }
    }
    
    private func rotateAndPerspectiveTransformForView(view: UIView) -> CATransform3D{
        setAnchorPoint(CGPointMake(0.5, 1.0), forView: view)
    ...
    private func rectForBorder(bounds: CGRect, isFilled: Bool) -> CGRect{
        return CGRect(x: 0, y: 0, width: bounds.size.width, height: bounds.size.height - font!.lineHeight + textFieldInsets.y - borderThickness)
    }
    ...
    private func updateForeground(){
        ...
        foregroundView.layer.transform = rotateAndPerspectiveTransformForView(foregroundView)
    ...
    private func layoutPlaceholderInTextRect(){
        ...
        placeholderLabel.frame = CGRect(x: originX, y: bounds.height - placeholderLabel.frame.height, width: placeholderLabel.frame.size.width, height: placeholderLabel.frame.size.height)
    }
